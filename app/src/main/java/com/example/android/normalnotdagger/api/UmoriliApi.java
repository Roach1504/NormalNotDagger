...
public interface UmoriliApi {
    ...
    @GET("api/get-posts")
    Call<NewsModel> getData(@Query("limit") String limit, @Query("offset") int offset, @Query("user_id") String user_id);//полдучения\ постов

    @GET("/api/set-mark")
    Call<StatusModel> addLike(@Query("id") String id, @Query("post_id") String post_id, @Query("mark") int mark);//установка лайка

    @GET("/api/set-view")
    Call<StatusModel> addView(@Query("id") String id, @Query("post_id") String post_id);//установка просмотра
    ...
    @GET("/api/sign-in")
    Call<RegistModel> getAvtor(@Query("login") String login, @Query("pass") String pass);
